...
class BookitgoldControllerRoom extends BookitgoldController
{
	...
	function remove()
	{
		...
		if(!$model->delete()) {
			$msg = JText::_( 'Error: One or More Room Could not be Deleted' );
		} else {
			$msg = JText::_( 'Room(s) Deleted' );
		}

		$this->setRedirect( 'index.php?option=com_bookitgold&controller=room&task=listing', $msg );
	}
	...
	function copy()
	{
		...
		$model = $this->getModel('room');
		if(!$model->copy()) {
			$msg = JText::_( 'Error: One or More Rooms Could not be Copied' );
		} else {
			$msg = JText::_( 'Room(s) Copied' );
		}

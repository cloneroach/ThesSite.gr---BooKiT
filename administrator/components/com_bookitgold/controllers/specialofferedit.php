...
class BookitgoldControllerSpecialofferedit extends BookitgoldController

{
...

	{
		...
		$model = $this->getModel('specialofferedit');

		$res = $model->store();

	

		$link = 'index.php?option=com_bookitgold&controller=specialoffer&task=edit&cid[]='.$res["id"];

		if ($res["code"]==0) {



			$msg = JText::_( 'Special Offer Saved!' );

			$this->setRedirect($link, $msg);

		} else if ($res["code"]==-2) {

			$msg = JText::_( 'Error Saving Special Offer.' );
			...
			$msg = JText::_( 'Please complete all basic fields.' );
...

	{
		...
		if ($res["code"]==0) {
			...
			$link = 'index.php?option=com_bookitgold&controller=specialoffer&task=listing';

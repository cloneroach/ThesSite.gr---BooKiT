...
class BookitgoldControllerRoomextraedit extends BookitgoldController

{
...

	{
		...
		$model = $this->getModel('roomextraedit');

		$res = $model->store();

		

		$link = 'index.php?option=com_bookitgold&controller=roomextra&task=edit&cid[]='.$res["id"];
		...
		if ($res["code"]==0) {
			...
			$msg = JText::_( 'Please complete all basic fields.Value must be a valid number.' );
...

	{
		...
		if ($res["code"]==0) {
			...
			$link = 'index.php?option=com_bookitgold&controller=roomextra&task=listing';
...

	{
		...
		JRequest::setVar( 'view', 'roomextra' );

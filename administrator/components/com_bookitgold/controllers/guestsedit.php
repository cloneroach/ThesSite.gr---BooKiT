...
class BookitgoldControllerGuestsedit extends BookitgoldController

{
...

	{
		...
		$model = $this->getModel('guestsedit');

		$res = $model->store();

	

		$link = 'index.php?option=com_bookitgold&controller=guests&task=edit&cid[]='.$res["id"];

		if ($res["code"]==0) {



			$msg = JText::_( 'Guest Saved' )."!";
			...
			$msg = JText::_( 'Error Saving Guest' );
...

	{
		...
		if ($res["code"]==0) {
			...
			$link = 'index.php?option=com_bookitgold&controller=guests&task=listing';

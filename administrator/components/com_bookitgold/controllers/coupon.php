...
class BookitgoldControllerCoupon extends BookitgoldController

{
...

	{
		...
		JRequest::setVar( 'view', 'couponedit' );
...

	{
		...
		if(!$model->delete()) {
			...
			$msg = JText::_( 'Coupon(s) Deleted' );

		}



		$this->setRedirect( 'index.php?option=com_bookitgold&controller=coupon&task=listing', $msg );
...

	{
		...
		if(!$model->copy()) {
			...
			$msg = JText::_( 'Error: One or More Coupons Could not be Copied' );

		} else {

			$msg = JText::_( 'Coupon(s) Copied' );

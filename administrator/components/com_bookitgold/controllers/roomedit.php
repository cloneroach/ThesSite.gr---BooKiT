...
class BookitgoldControllerRoomedit extends BookitgoldController

{
...

	{
		...
		$model = $this->getModel('roomedit');

		$res = $model->store();

		$link = 'index.php?option=com_bookitgold&controller=room&task=edit&cid[]='.$res["id"];
		...
		if ($res["code"]==0) {
			...
			$msg = JText::_( 'Please complete all basic fields. You have to create a category before saving a room' );
...

	{
		...
		if ($res["code"]==0) {
			...
			$link = 'index.php?option=com_bookitgold&controller=room&task=listing';
			...
			$msg = JText::_( 'Please complete give a name and choose a room category' );

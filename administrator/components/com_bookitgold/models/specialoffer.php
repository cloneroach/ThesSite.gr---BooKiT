...
class BookitgoldModelSpecialoffer extends JModel
{
	var $_data;
	var $_total = null;
	...
	{
		$query = ' SELECT * '
		. ' FROM #__bookitspecialoffer '.$this->_buildQueryWhere(). ' ' .
...
'search', '', 'string');
	...
	function _buildCopyQuery($id)
	{
		...
		$db->insertObject( '#__bookitspecialoffer', $data, 'idoffer' );
		return true;
	}
